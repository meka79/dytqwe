...
# ==============================================================================
# 1. TAB: DANIŞAN KABUL & ANALİZ (RENKLİ GÖRÜNÜM İÇİN DÜZENLENDİ)
# ==============================================================================
if menu == "1. Danışan Kabul & Analiz":
    ...
    with col2:
        ...
        # 1 kg ~ 7700 kcal, hedef 90 gün (3 ay) baz alınmıştır.
        # Haftalık ortalama 0.5 kg hedefiyle (7700*0.5/7) ~ 550 kcal günlük açık/fazla oluşturulur
        kalori_farki = fark * 7700 / 90 
        hedef_kalori = round(analiz['tdee'] + kalori_farki)
        
        st.markdown("---")
        st.subheader("Hesaplama Sonuçları")
        
        # --- BAŞLANGIÇ GÖRÜNÜMÜ: RENKLİ KARTLAR (İstenen Model) ---
        c_r1, c_r2, c_r3 = st.columns(3)

        # BMI Card (Mavi)
        c_r1.markdown(f"""
            <div style="background-color: #e6f7ff; border-left: 5px solid #007bff; padding: 10px; border-radius: 5px;">
                <p style="margin: 0; font-size: 14px; color: #007bff;">BMI (Vücut Kitle İndeksi)</p>
                <h3 style="margin: 0; color: #007bff;">{analiz['bmi']}</h3>
            </div>
        """, unsafe_allow_html=True)

        # BMH Card (Mor/Fuşya)
        c_r2.markdown(f"""
            <div style="background-color: #f7e6ff; border-left: 5px solid #8e44ad; padding: 10px; border-radius: 5px;">
                <p style="margin: 0; font-size: 14px; color: #8e44ad;">BMH (Bazal Metabolizma)</p>
                <h3 style="margin: 0; color: #8e44ad;">{analiz['bmh']} kcal</h3>
            </div>
        """, unsafe_allow_html=True)
        
        # TDEE Card (Turuncu)
        c_r3.markdown(f"""
            <div style="background-color: #fff8e6; border-left: 5px solid #ffaa00; padding: 10px; border-radius: 5px;">
                <p style="margin: 0; font-size: 14px; color: #ffaa00;">TDEE (Günlük İhtiyaç)</p>
                <h3 style="margin: 0; color: #ffaa00;">{analiz['tdee']} kcal</h3>
            </div>
        """, unsafe_allow_html=True)
        
        st.markdown("<br>", unsafe_allow_html=True) # Boşluk için

        # Kilit Sonuç Kartı: Diyet Kalorisi ve İdeal Kilo (Yeşil)
        st.markdown(f"""
            <div style="background-color: #e6fff0; border: 2px solid #4CAF50; padding: 15px; border-radius: 10px; text-align: center;">
                <h2 style="margin: 0; color: #4CAF50;">{hedef_kalori} kcal</h2>
                <p style="margin: 0; font-size: 18px; opacity: 0.8; color: #4CAF50;">Önerilen Günlük Diyet Kalorisi</p>
                <hr style="border-color: #4CAF50; margin: 15px 0;">
                <p style="font-size: 16px;">İdeal Kilo Aralığı: <b>{analiz['ideal_min']} kg - {analiz['ideal_max']} kg</b></p>
            </div>
        """, unsafe_allow_html=True)
        # --- RENKLİ KARTLAR SONU ---

        planlanan_kalori = st.number_input("Planlanan Kalori (Otomatik Öneri: {} kcal)".format(hedef_kalori), value=hedef_kalori, step=50)
...
# ==============================================================================
    ...
    # Anamnez Testi Grafiği (v13'ten alınmıştır)
    with c_g3:
        ...
        if secilen_danisan != "Seçiniz...":
            ...
            if st.button("Testi Bitir ve Kaydet", type="primary"):
                ...
                try:
                    ...
                    # Sonuca göre hızlı değerlendirme (st.error/warning/info kullanılarak renkli çıktı)
                    if toplam_skor >= 15:
